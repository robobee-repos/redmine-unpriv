...
function start_redmine() {
  ...
  isLikelyRedmine=
  case "$1" in
    puma | rails | rake | passenger ) isLikelyRedmine=1 ;;
  esac
  ...
  if [ -n "$isLikelyRedmine" ]; then
    _fix_permissions
    if [ ! -f './config/database.yml' ]; then # test ./config/database.yml
      ...
      if [ "$REDMINE_DB_MYSQL" ]; then # if database
        adapter='mysql2'
